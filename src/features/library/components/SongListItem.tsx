  ...
  return (
    <button
      ...
      className={cn(
        ...
        isPlaying ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-950/35 dark:text-emerald-300" : "text-slate-700 dark:text-slate-200",
      )}
    >
      <div className="mr-3 flex min-w-0 items-center gap-3">
        <span className={cn(
          "w-5 text-xs tabular-nums text-slate-400 group-hover:text-emerald-500 transition-colors",
          isPlaying && "text-emerald-500 font-bold"
        )}>
          {index + 1}
        </span>
